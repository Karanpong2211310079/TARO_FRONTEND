        ...
        <div className="player-cards-modal" onClick={onClose}>
            <div className="player-cards-content" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => { playSound('pop'); onClose(); }} className="text-yellow-400 hover:text-yellow-500 text-xl font-bold">×</button>
                </div>
                    ...
                    {playerCards.cards.map((card, index) => (
                        <div key={`${card.name}-${index}`} className="card-item">
                            <div className="flex-1">
                                ...
                                <button className="use-card-btn mt-1" onClick={() => { playSound('pop'); onUseCard(index); }}>ใช้</button>
                            </div>
                        </div>
                    ))}
                </div>
